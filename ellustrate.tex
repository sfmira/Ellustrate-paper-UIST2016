...
\section{Systems Details}
  ...
  % Within Ellustrate Validate cycle, the digital tool not only checks for traditional mistakes such as misplaced ground connections, but also problems that are unique to sketched circuits, such as overly long and thin traces made with the relatively less conductive crafting materials (i.e. silver, graphite, conductive threads). After the circuit design is validated, Ellustrate Fabricate mode will provide users with step-by-step instructions on fabricating the circuit and debugging problems as they arise. 
    ...
    % \cesar{The biggest insight here that I want to convey here is as follows: 
    %   Yes, at a high level, this graph is simply us traversing the paths like a visual tree. Connections are vertexes. 
    %   However, when you do this with a drawing, the ``branches'' of the tree are tiny. Imagine an intersection of two paths end-to-end.
    %   This yields tiny branches, although we understand this to be a single connection. By doing the above, we use a visual property of closeness to abstract away these tiny branches. 
    %   }
      ...
      Through a set of post-processing steps, we extract a graph representation of the circuit. We use $n$ to refer to the number of paths post-processed.
      % (e.g. loops $\ell$, crosses)
      For each path intersection $s$ and self-intersection $t$, relevant paths are sliced at that intersection yielding $2(s + t)$ new paths. Intersections are disregarded is they occur at the start or end of the path, or if the two paths in question are collinear.
      For each blob intersection $b$, we draw a line from the blob center to the boundary intersection point yielding $b$ new paths.
      We then build an undirected $G$ using an adjacency list $\langle V, E \rangle$. A vertex on the graph is every start and end of each path ($v = 2n = 4(s + t) + 2p + 2b $). An edge is defined as a connection where two vertices are connected by a path or an intersection with another vertex; each edge is encoded with its position on the canvas, material composition, and cross-sectional area.
          % TOTAL VERTICES V = 4(s+t) + 2p + 2b
          % AFTER OPTIMIZATION V = s + t + b + 2p

      \textit{Optimization step}. We uses the physical proximity of each vertex to simplify the graph. Vertices which are close together (e.g. four vertices at an intersection between two paths) are joined, reducing the total number of vertices by at most $3(s + t) + b$. This optimization reduces the number of vertices to $s + t + b + 2p$. To give context, this optimization allows us to work with a 86 vertex graph of the octopus in Figure \ref{fig:teaser}a (b = 32, s = 9, t = 17, p = 14) compared to a 196 vertex graph.
...
\subsection{Aesthetic and Electrical Planning}
  ...
  \jlo{ease of routing was a big factor that drove many designs, which was to be expected for people with no background in circuit design. users with some circuit background was able to use electronic elements as part of the design much more freely. This highlights the importance of technological fluency in creativity.}
  ...
  % users who drew primarily adherent lines more strictly followed the SVG when placing footprints. Yet, all participants, when placing LEDs, did try to place them in semiotically relevant places and let those LED placements guide the design. This is relevant in . 
...
%Jasper working on
 ...
 %Joanne working on 
  ...
  \jlo{want to understand why the resistance value for checking changes}
  \jlo{while some users appreciate the step-by-step guide, some questioned the format. When the researcher explained that this was a method provided by other experts for easier debugging, they were much more likely to be accepting of the guide. Some of them continued to brainstorm for a better way to fabricate such that the process is more free-flowing but still allows for easy debug. We were excited to see that users move beyond an information absorbing stage and try to synthesize ways to become more expert like. Everyone learns the process differently, so this also inspire us to enable people to post their own fabrication and debug guides. }
