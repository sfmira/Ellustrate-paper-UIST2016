...
% \ep{sure but make it clear that it is somewhat hardware independent since it is broswer based}
    ...
    we use an Apple iPad Pro and Apple Pencil, chosen to emulate a pen and paper design environment. Ellustrate uses the paper.js vector graphics scripting framework~\cite{lehni_paperjs_2011} which enables common vector editing operations and  backwards compatability with vector graphics applications. Interactions followed noun-verb drawing application conventions (i.e., click on action icon, carry out action). At a high level, the tool exposes to users two operations: the ability to lay down components, and the ability to make marks representing different conductive materials to connect components.
