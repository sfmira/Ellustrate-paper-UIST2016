...
% \newcommand*{\ohm}[1]{\ensuremath{\Upomega}}
...
\section{Motivation}
Drawing and sketching play a critical role within the development of user interfaces. Sketching has shown to bring interesting elements to hardware prototyping, sparking creative innovations within the Maker community. Although sketching circuit has many benefits in initial design and early electronic education, it is not explored by supported by many electrical digital design tools. \cesar{can we make figure \ref{fig:comparison_table} have a narrower aspect ratio. And have the checkmarks/x's take up slightly less space in the grid. (more padding). Also, and we do a --- for "feature not supported".}
...
%\subsection{Digital Apprenticeship}
...
\subsection{Participants}
    ...
    % The same group of participants was used for both experiments. We conducted each experiment with 10 participants who had no prior experience with circuit design but were well versed in visual design. Visual design prowess was determined from self-reported expertise. Participants were recruited from a mailing list of Architecture, Art, and Design students at our institution and from the surrounding community via Craigslist. 
    % (might add: Additionally, 10 participants from technical fields (i.e. computer sciences, mechanical engineering, material sciences) who had little to no prior experience with circuit design were recruited. Experience with circuit design was determined from self-reported feedback. Participants were recruited from a mailing list of technical departments.)
    % A tutorial was presented in slideshow format to participants in the beginning of the study to familiarize them with introductory concepts of circuit design, topics include resistance, polarity of electronic components and power, and simple debugging with a multimeter. The tutorial  presentation was available as a reference throughout the study. The participants were then guided by the researcher to fabricate a simple, one-LED circuit powered by a coin cell battery as a warm up exercise.   
