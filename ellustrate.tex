    ...
    \subsection{Post-design validation}
        We run a set of simple validation rules against the extracted circuit graph. These checks produced with error, warning, or success indicators in a validation side panel. (Figure \ref{ellustrate_system}). 
            \begin{itemize}
                ...
                \item \factor{FAB CHECK}. For each path, verify against fabrication-specific constraints (e.g. length of trace for silver-ink will be difficult to make).
            \end{itemize}
...
\section{Fabrication Tool}
    ...
    \subsection{Design-specific fabrication instructions}
        ...
        \begin{itemize}   
          \item {[}Dry{]} Material-specific drying times (e.g. graphite ink takes longer to dry than silver ink). 
...
\centering
  ...
  \caption{\jasper{Retake this figure to match refs below} Completed circuits from the user study drawn in silver ink on paper. Circuit designs show examples of a) functionalist, b) mimetic, c) constructive, and d) metaphorical marks.}~\label{fig:user-artwork}
  \vspace{-20pt}
\end{figure}
